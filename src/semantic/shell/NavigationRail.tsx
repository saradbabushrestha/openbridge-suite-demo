...
export function NavigationRail({ mode = 'rail', behavior = 'regular', title = 'Navigation menu', size = '100%', caption }: NavigationRailProps) {
  const variantMap = map[mode];
  const Asset = (variantMap as any)[behavior] ?? variantMap.regular;
  return (
    <OpenBridgeFrame title={title} subtitle={`${mode} · ${behavior}`} footer={caption}>
      <Asset width={size} responsive title={title} />
    </OpenBridgeFrame>
  );
}
