...
export function AdviceButton({ variant = 'flat', state = 'enabled', active = false, title = 'Advice', size = 48, className, onClick }: AdviceButtonProps) {
  const Asset = variant === 'flat' ? flatMap[state][String(active) as 'false' | 'true'] : variant === 'normal' ? normalMap[state] : enhancedMap[state];
  return (
    <button type='button' aria-label={title} title={title} onClick={onClick} className={cn('ob-advice-button', className)} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 0, padding: 0, cursor: 'pointer' }}>
