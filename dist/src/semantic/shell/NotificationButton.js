...
export function NotificationButton({ variant = 'flat', state = 'enabled', active = false, count, title = 'Notifications', size = 48, className, onClick }) {
    const Asset = variant === 'flat' ? flatMap[state][String(active)] : variant === 'normal' ? normalMap[state] : enhancedMap[state];
    return (_jsxs("button", { type: 'button', "aria-label": title, title: title, onClick: onClick, className: cn('ob-notification-button', className), style: { position: 'relative', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 0, padding: 0, cursor: 'pointer' }, children: [_jsx(Asset, { size: size, title: title }), typeof count === 'number' && count > 0 ? (_jsx("span", { style: { position: 'absolute', right: -2, top: -2, minWidth: 18, height: 18, borderRadius: 999, background: '#cf473b', color: '#fff', fontSize: 11, lineHeight: '18px', textAlign: 'center', padding: '0 5px', fontWeight: 700 }, children: count > 99 ? '99+' : count })) : null] }));
}
