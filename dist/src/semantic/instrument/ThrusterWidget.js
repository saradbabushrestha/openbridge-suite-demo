...
export function ThrusterWidget({ priority = 'regular', styleMode = 'bar', state = 'active', thrust = 64, title = 'Tunnel thruster' }) {
    const Asset = map[priority][styleMode][state];
    return (_jsx(OpenBridgeFrame, { title: title, subtitle: `${priority} · ${styleMode} · ${state}`, children: _jsxs("div", { style: { display: 'grid', placeItems: 'center', gap: 10 }, children: [_jsx(Asset, { responsive: true, width: '100%', title: title }), _jsx(MetricText, { value: thrust, unit: '%', secondary: 'Applied thrust' })] }) }));
}
